...
<body>
	<h1>지점 상세</h1>
	<form action="<c:url value="/hq/branch/update"/>" method="post" enctype="multipart/form-data" id="form">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" id="br_address" name="br_address" value="${br.br_address}">
		</div>
		...
		<button class="btn btn-outline-warning col-12">지점 상세 수정</button>
	</form>
	<hr/>
	...
	<script>
		$(document).ready(function(){
		    ...
		    var size = ${bfList.size()};
			console.log(size);
			for(var i = 0; i < size; i++){
				let url = "${bfList.get("+"\${i}"+").bf_name}"; 
				let file = convertURLtoFile(url);
