		...
		try {
		    ...
		    if ("paid".equals(payment.getStatus())) {
				...
				if(res) {
				    ...
				    response.put("url", "/membership/membershipList"); // 성공 URL 추가
				}
				else {
				    response.put("success", false);
				    response.put("message", "결제가 실패하였습니다.");
				    response.put("url", "/membership/membershipInsert"); // 실패 URL 추가
				}
			}else {
				response.put("success", false);
		        response.put("message", "결제가 실패하였습니다. 상태: " + payment.getStatus());
		        response.put("url", "/membership/membershipInsert");
			}
		}catch (Exception e) {
	        response.put("success", false);
